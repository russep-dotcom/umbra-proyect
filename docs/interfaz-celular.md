...
- Mini-menú de texto abajo (`#estilo-pop`: A−/A+, FUENTE, COLOR) con paneles amplios arriba (`#sub-fuente` con NORMAL/MÁQUINA/EXPEDIENTE/TERROR/SANGRE; `#sub-color` con familias, TONOS por familia — normal y blanco dicen «no hay variantes de este color» — y CÓDIGO hex que también da tonos): abre con la A y al tocar texto escrito (lo re-selecciona y aplica), sin margen jamás en celular; lo elegido aplica al texto y se hereda al siguiente; al abrir el teclado los paneles se cierran y la base se deja tapar; tocar sus botones no mata la pendiente. NORMAL hereda la página y MÁQUINA es Courier New (toda la página ya es Special Elite, por eso se veían iguales).
