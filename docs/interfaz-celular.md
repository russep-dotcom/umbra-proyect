...
- Herramienta de texto estilo excalidraw (botón A en la barra, no existe en computador): A y luego tablero para escribir solo la barrita, sin recuadro ni ✕; cada toque reubica la barra en el dedo (la vacía anterior desaparece); si ya hay texto se queda y la herramienta se apaga (también se apaga con la A); el texto vacío nunca muestra margen con puntos y al confirmar la margen se ajusta a su largo (el texto en celular no se borra con ✕).
